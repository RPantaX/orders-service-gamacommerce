...
echo "🐳 Docker Build & Push Script para orders Service"
echo "=================================================="
echo ""
...
CONTAINER_ID=$(docker run -d -p 8084:8081 --name orders-service-test "$VERSIONED_TAG")
...
if [ $? -eq 0 ]; then
    ...
    if docker ps | grep -q "orders-service-test"; then
        print_success "Contenedor ejecutándose correctamente"

        # Intentar hacer health check
        print_info "Verificando health check..."
        if docker exec orders-service-test curl -f http://localhost:8001/actuator/health 2>/dev/null; then
        ...
        docker logs --tail 10 orders-service-test

    else
        print_error "El contenedor se detuvo inesperadamente"
        print_info "Logs del contenedor:"
        docker logs orders-service-test
    fi

    # Limpiar contenedor de prueba
    print_info "Limpiando contenedor de prueba..."
    docker stop orders-service-test >/dev/null 2>&1 || true
    docker rm orders-service-test >/dev/null 2>&1 || true
